...
public partial class LargeDataFeed : MonoBehaviour, IComparer<DoubleVector2>
{
    ...
    List<DoubleVector2> mData = new List<DoubleVector2>();  // the devicesInfo held by the chart
    double pageSize = 2f;
    ...
    /// <summary>
    /// called with Start(). These will be used to load the devicesInfo into the large devicesInfo feed. You should replace this with your own loading logic.
    /// </summary> 
    void SetInitialData()
    {
        ...
        double x = 0f;
        double y = 200f;
        for (int i = 0; i < 25000; i++)    // initialize with random devicesInfo
        {
    ...
    /// <summary>
    /// vertify's that the graph devicesInfo is sorted so it can be searched using a binary search.
    /// </summary>
    /// <returns></returns>
    ...
    /// <summary>
    /// set the devicesInfo of the large devicesInfo graph
    /// </summary>
    ...
    public void SetData(List<DoubleVector2> data)
    {
        ...
        {
            Debug.LogWarning("The devicesInfo used with large devicesInfo feed must be sorted acoording to the x value, aborting operation");
            return;
        }
        mData = data;
        OnDataLoaded();
    ...
    {
        //NOTE :: this method assumes your devicesInfo is sorted !!! 
    ...
    void findPointsForPage(double position, out int start, out int end) // given a page position , find the right most and left most indices in the devicesInfo for that page. 
    {
        int index = FindClosestIndex(position); // use binary search to find the closest position to the current scroll point
    ...
    {
        for (int i = start; i < end; i++) // load the devicesInfo
        {
