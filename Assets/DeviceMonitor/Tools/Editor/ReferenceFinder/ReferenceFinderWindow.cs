...
public class ReferenceFinderWindow : EditorWindow
{
    ...
    public static ReferenceFinderData GetFinderData()
    {
        return data;
    }
...
/*
    [MenuItem("Assets/Export References In Project")]
    static void ExportRef()
    {
        ...
        if (Selection.activeObject != null)
        {
            ...
            // 收集选中的guid
            //
            var selectedAssetGuid = new List<string>();
            ...
            foreach (var guid in selectedAssetGuid)
            {
                if (data.assetDict.TryGetValue(guid, out var desc))
                {
                    var stack = new Stack<string>();
                    foreach (var child in desc.dependencies)
                    {
                        ExportDeps(child, deps, stack);
                    }
                    
                    stack.Clear();
                    foreach (var child in desc.references)
                    {
                        ExportRefs(child, refs, stack);
                    }
                }
            }
            ...
            {
                sb.AppendLine(path);
                sb.AppendLine("引用以下资源：");
                
                var depList = deps.ToList().ConvertAll(AssetDatabase.GUIDToAssetPath);
                depList.Sort();
                int index = 0;
            ...
            if (refs.Count > 0)
            {
                sb.AppendLine();
                sb.AppendLine(path);
                sb.AppendLine("被以下资源引用：");

                var refList = refs.ToList().ConvertAll(AssetDatabase.GUIDToAssetPath);
                refList.Sort();
                int index = 0;
            ...
            var logfile = Path.Combine(Application.temporaryCachePath, "TempLog.txt");
            File.WriteAllText(logfile, sb.ToString());
            UnityEditor.EditorUtility.OpenWithDefaultApp(logfile);
        }
    }

    static void ExportDeps(string guid, HashSet<string> deps, Stack<string> stack)
    {
    ...
    static void ExportRefs(string guid, HashSet<string> refs, Stack<string> stack)
    {
        if (stack.Contains(guid))
            return;
        stack.Push(guid);
    ...
    */
    ...
    public void DrawOptionBar()
    {
        ...
        isDepend = GUILayout.Toggle(isDepend, isDepend ? "Model(Depend)" : "Model(Reference)", toolbarButtonGUIStyle,GUILayout.Width(200));
