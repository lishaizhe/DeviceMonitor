...
public class AssetTreeView : TreeView
{
    ...
    {
        var selectedNodes = new List<AssetViewItem>();
        foreach (var nodeId in GetSelection())
        {
            var item = (AssetViewItem) FindItem(nodeId, rootItem);
            if (item != null)
            {
        ...
        if (selectedNodes.Count > 0)
        {
            ...
            menu.AddItem(new GUIContent("Remove All Old References"), false, _ =>
            {
                for (int i = 0; i < selectedNodes.Count; i++)
                {
                    var material = AssetDatabase.LoadAssetAtPath<Material>(selectedNodes[i].data.path);
                    if (material != null)
                    {
                        RemoveAllUnusedProperties(material);
    ...
    private void RemoveAllUnusedProperties(Material material)
    {
        var serializedObject = new SerializedObject(material);
        RemoveUnusedProperties("m_SavedProperties.m_TexEnvs", material, serializedObject);
        RemoveUnusedProperties("m_SavedProperties.m_Floats", material, serializedObject);
        RemoveUnusedProperties("m_SavedProperties.m_Colors", material, serializedObject);
    }
    private void RemoveUnusedProperties(string path, Material material, SerializedObject serializedObject)
    {
