...
namespace UnityGameFramework.Runtime
{
    public partial class DebuggerComponent
    {
        ...
        private sealed partial class ConsoleWindow : IDebuggerWindow
        {
            ...
            private SettingProxy m_SettingComponent = null;
            ...
            private bool m_LastToggleLog = false;
            ...
            GUIStyle m_toggleStyle = new GUIStyle();

            [SerializeField]
            private bool m_ToggleLog = false;
            
            [SerializeField]
            private bool m_LockScroll = true;

            [SerializeField]
            private int m_MaxLine = 500;
            ...
            {
                m_SettingComponent = GameEntry.Setting;
...
#if !UNITY_EDITOR                
                m_ToggleLog = m_LastToggleLog = true;//false;
#else
                m_ToggleLog = m_LastToggleLog = true;
#endif                
                
                m_LockScroll = m_LastLockScroll = m_SettingComponent.GetPublicBool("Debugger.Console.LockScroll", true);
                m_InfoFilter = m_LastInfoFilter = m_SettingComponent.GetPublicBool("Debugger.Console.InfoFilter", true);
                m_WarningFilter = m_LastWarningFilter = m_SettingComponent.GetPublicBool("Debugger.Console.WarningFilter", true);
                m_ErrorFilter = m_LastErrorFilter = m_SettingComponent.GetPublicBool("Debugger.Console.ErrorFilter", true);
                m_FatalFilter = m_LastFatalFilter = m_SettingComponent.GetPublicBool("Debugger.Console.FatalFilter", true);

                m_guiStyle.fontSize = 22;

                m_toggleStyle.fixedWidth = 90;
                m_toggleStyle.fixedHeight = 28;
                m_toggleStyle.fontSize = 16;
            }
            ...
            {
                if (m_LastToggleLog != m_ToggleLog)
                {
                    m_LastToggleLog = m_ToggleLog;
                    Debug.unityLogger.logEnabled = m_ToggleLog;
                }
                
                if (m_LastLockScroll != m_LockScroll)
                {
                    m_LastLockScroll = m_LockScroll;
                    m_SettingComponent.SetPublicBool("Debugger.Console.LockScroll", m_LockScroll);
                }

                if (m_LastInfoFilter != m_InfoFilter)
                {
                    m_LastInfoFilter = m_InfoFilter;
                    m_SettingComponent.SetPublicBool("Debugger.Console.InfoFilter", m_InfoFilter);
                }

                if (m_LastWarningFilter != m_WarningFilter)
                {
                    m_LastWarningFilter = m_WarningFilter;
                    m_SettingComponent.SetPublicBool("Debugger.Console.WarningFilter", m_WarningFilter);
                }

                if (m_LastErrorFilter != m_ErrorFilter)
                {
                    m_LastErrorFilter = m_ErrorFilter;
                    m_SettingComponent.SetPublicBool("Debugger.Console.ErrorFilter", m_ErrorFilter);
                }

                if (m_LastFatalFilter != m_FatalFilter)
                {
                    m_LastFatalFilter = m_FatalFilter;
                    m_SettingComponent.SetPublicBool("Debugger.Console.FatalFilter", m_FatalFilter);
                }
            }
            ...
            public void OnDraw()
            {
                ...
                {
                    m_toggleStyle.fixedWidth = 300;
                    GUILayout.Space(20);
                    m_ToggleLog = GUILayout.Toggle(m_ToggleLog, ">>>> Toggle Log  <<<<");
                    m_LockScroll = GUILayout.Toggle(m_LockScroll, "Lock Scroll");
                    GUILayout.FlexibleSpace();

                    m_toggleStyle.fixedWidth = 90;
                    
                    m_InfoFilter = GUILayout.Toggle(m_InfoFilter, string.Format("Info ({0})", m_InfoCount.ToString()), m_toggleStyle);
                    m_WarningFilter = GUILayout.Toggle(m_WarningFilter, string.Format("Warning ({0})", m_WarningCount.ToString()), m_toggleStyle);
                    m_ErrorFilter = GUILayout.Toggle(m_ErrorFilter, string.Format("Error ({0})", m_ErrorCount.ToString()), m_toggleStyle);
                    m_FatalFilter = GUILayout.Toggle(m_FatalFilter, string.Format("Fatal ({0})", m_FatalCount.ToString()), m_toggleStyle);

                    if (GUILayout.Button("Clear All", GUILayout.Width(100f), GUILayout.Height(28f)))
                    {
                        Clear();
                    }
                }
                ...
                GUILayout.BeginVertical("box");
                {
                    ...
                    m_LogScrollPosition = GUILayout.BeginScrollView(m_LogScrollPosition);
                    {
                        ...
                        for (LinkedListNode<LogNode> i = m_Logs.First; i != null; i = i.Next)
                        {
                            ...
                            if (GUILayout.Toggle(m_SelectedNode == i, GetLogString(i.Value), m_guiStyle))
                            {
                                selected = true;
                                if (m_SelectedNode != i)
                                {
                ...
                GUILayout.BeginVertical("box");
                {
                    m_StackScrollPosition = GUILayout.BeginScrollView(m_StackScrollPosition, GUILayout.Height(70f));
                    {
