...
#region Request Callbacks
        ...
        private void OnInternalRequestCallback(HTTPRequest req, HTTPResponse resp)
        {
            ...
            switch (req.State)
            {
                case HTTPRequestStates.Finished:
                    if (resp.IsSuccess || resp.StatusCode == 101)
                    {
                        ...
                        //这个有可能是谷歌服务器转发造成的，尝试重连会连上
                        if (resp.StatusCode == 101)
                        {
                            OnError?.Invoke(this,new Exception("serverError"));
                            OnErrorDesc?.Invoke(this, "serverError");
                        }
                        return;
                    }
                    else
            ...
            if (this.State != WebSocketStates.Connecting || !string.IsNullOrEmpty(reason))
            {
                ...
                if (OnError == null && OnErrorDesc == null && (req.State != HTTPRequestStates.Aborted))
                    HTTPManager.Logger.Warning("WebSocket", reason);
            }
